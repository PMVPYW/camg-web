    ...
    <div
      ...
      <div class="relative">
        <a class="block shadow-xl rounded-2xl bg-[#f8f9fe] h-64">
          <img v-if="props.album.img" :src="`${serverBaseUrl}/storage/fotos/${props.album.img}`" :alt="`${serverBaseUrl}/storage/fotos/${props.album.img}`"
      ...
      <div class="flex-auto px-1 pt-6">
        <p class="relative z-10 mb-2 leading-normal text-transparent bg-slate-700 text-sm bg-clip-text">
          Rally: {{ props.album.rally_id == null ? 'Sem Rally' : rallyStore.rallies.find((item) => item.id === props.album.rally_id)}}</p>
        <a href="javascript:;">
