...
<template>
  ...
  <div class="w-11/12 my-8 h-full rounded-lg mx-auto p-4">
    <ConselhoSeguranca  @click="()=>setSelectedConselho(cs)" v-for="cs in conselhoSegurancaStore.conselhos_seguranca" :key="cs.id" :conselhoSeguranca="cs" :obj_to_edit="selected_conselho" class="border-2 rounded-xl" :class="{'bg-gradient-to-br from-[#F3AA06] to-[#997A2E]': selected_conselho.id==cs.id}"></ConselhoSeguranca>
  </div>
</template>
