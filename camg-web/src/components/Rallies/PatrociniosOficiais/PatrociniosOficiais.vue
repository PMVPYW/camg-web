...
<template>
 <div v-if="rallyStore.rally_selected" class="h-full w-full rounded-xl transition-all duration-200 overflow-x-hidden" id="panel">
  ...
  <div class="flex flex-col w-11/12 mx-auto mt-16 rounded-lg justify-center bg-[#f8f9fe]">
   <div class="flex flex-col bg-[#f8f9fe] w-full h-16 justify-center">
    <div class="flex flex-row items-center justify-center w-full my-10">
     <button @click="()=>{associating=!associating;editing = false; selectedPatrocinio=!selectedPatrocinio}" type="button"
     ...
     <button @click="()=>{associating=false;!selectedPatrocinio.id ? editing = false : editing = !editing}" type="button" :disabled="!selectedPatrocinio.id"
   ...
   <div v-if="editing === true" class="w-full">
    ...
    <div class="flex flex-row flex-wrap items-center justify-center w-full mt-16">
      ...
      <div class="flex flex-row items-center mb-2">
        ...
        <select v-model="order_by" class="font-bold py-3 px-4 w-48 block text-slate-700 bg-gray-100 text-sm rounded-lg border-b-2 border-amber-200 focus:border-amber-400 focus:ring-amber-500">
          <option value="nome_asc">Nome de Z-A</option>
          <option value="nome_desc">Nome de A-Z</option>
        </select>
      </div>
    </div>
  </div>
