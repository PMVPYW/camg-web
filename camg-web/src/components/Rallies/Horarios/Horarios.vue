<script setup>
import HorariosTable from "@/components/Rallies/Horarios/HorariosTable.vue";
...
<template>
 <div v-if="rallyStore.rally_selected" class="h-dvh rounded-xl transition-all duration-200 w-full max-h-dvh" id="panel">
  <h1 class="text-2xl font-bold ml-10 mt-10">Horários</h1>
  <HorariosTable class="w-full h-[80%] overflow-y-auto"></HorariosTable>
 </div>
 <div v-else class="mx-auto mt-12">
  <div class="flex flex-row items-center bg-gray-100 border border-amber-400 py-2 px-4 rounded-xl">
   <h1 class="text-gray-500 text-base font-medium">
    Não existem rallys, para ver o horário de um rally terá que o criar primeiro!
   </h1>
   <div>
