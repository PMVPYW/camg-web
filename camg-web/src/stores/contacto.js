...
export const useContactoStore = defineStore("contacto", () => {
  ...
  socket.on("delete_tipocontacto", (tipocontacto) => {
    tipo_contactos.value = tipo_contactos.value.filter(
      (item) => item.id != tipocontacto,
    );
    tipo_contactos_filters.value = tipo_contactos_filters.value.filter(
      (item) => item.id != tipocontacto,
    );
  ...
  socket.on("update_tipocontacto", (tipocontacto) => {
    var index = tipo_contactos.value.findIndex(
      (item) => item.id === tipocontacto.id,
    );
    if (index >= 0) {
      tipo_contactos.value[index] = tipocontacto;
    }
    index = tipo_contactos_filters.value.findIndex(
      (item) => item.id === tipocontacto.id,
    );
    if (index >= 0) {
